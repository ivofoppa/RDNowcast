...
//' 
// [[Rcpp::export]]
IntegerVector Test(DataFrame data, Date dateAnal, int offset, DateVector NCdates, int NCsize = 10,  
...
for(DateVector::iterator d = analdates.begin(); d != analdates.end(); ++d) {
  ...
  DateVector refdatessub(evdates[ (evdates>dte) & (evdates<= Date(dte + mult)) & (rpdates < Date(dateAnal)) ]);
  countVec.push_back(refdatessub.length());
}
